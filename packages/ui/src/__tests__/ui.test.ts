...
describe("UI public assets", () => {
  ...
  it("should have character sprites for all roles", () => {
    ...
    for (const role of roles) {
      assert.ok(
        existsSync(join(uiRoot, "public", "assets", "sprites", `${role}.png`)),
        `Missing sprite: ${role}.png`
      );
      assert.ok(
        existsSync(join(uiRoot, "public", "assets", "sprites", `${role}.json`)),
        `Missing atlas: ${role}.json`
      );
    }
  });
